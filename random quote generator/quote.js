function generate(){
    const quotes = {//creating a dictionary with line of quotes and thier authotrs
    ...
    var authors = Object.keys(quotes);//here we are assessing the authors with the key() function. This function immidiately create and array
   // var texts = Object.values(quotes); //We did the same here but this line is absolutely not usefull


    var author = authors[Math.floor(Math.random()*authors.length)];//Now we want to acces each index of the array
    var text = quotes[author];//we don't make the same for text because we want the quote and the author to be in concordance
